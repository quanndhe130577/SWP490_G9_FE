...
export default class AdvanceSalary extends Component {
  constructor(props) {
    super(props);
  ...
  async fetchBasket() {
    try {
      let rs = await apis.getAllAdvanceSalary({}, "GET", this.props.match.params.id);
    ...
    return (
      <Row>
        <Col md="6" className="d-flex">
          <h3 className="">{i18n.t("advanceSalaryManagerment")}</h3>
  ...
  onClick(modeBtn, BasketID) {
    ...
    if (modeBtn === "edit") {
      ...
      helper.confirm(i18n.t("confirmDelete")).then(async (rs) => {
        if (rs) {
          try {
            let rs = await apis.deleteAdvanceSalary({}, "POST", `${BasketID}`);
            if (rs && rs.statusCode === 200) {
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        dataIndex: "debt",
        key: "debt",
        ...this.getColumnSearchProps("debt"),
