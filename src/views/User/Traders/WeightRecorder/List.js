...
export default class List extends Component {
    ...
    columns = [
        ...
        },
        {
            ...
            sortDirections: ["descend", "ascend"],
            render: (data) => {
                return <Checkbox onChange={event => this.changeChecked(event.target.checked, data.id)} checked={data.isAccepted}>{i18n.t("isAccepted")}</Checkbox>
            }
        },
        {
            title: i18n.t("action"),
            render: (data) => {
                return (
                    <>
                        {
                            data.canDelete && <Button color={data.isDeleted ? i18n.t("success") : i18n.t("danger")} onClick={() => this.delete(data.id)}>
